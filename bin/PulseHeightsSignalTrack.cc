...
int SlopeSelection (int Channel, PLTTrack* Track)
{
  ...
  switch (Channel)
    {
    case 13:
      if(slopeX>-0.014 && slopeX<-0.010 && slopeY>0.010 && slopeY<0.015) return 1;
      break;
    case 24:
      if(slopeX>0.018 && slopeX<0.023 && slopeY>-0.001 && slopeY<0.002) return 1;
      break;
    case 14:
      if(slopeX>-0.012 && slopeX<-0.008 && slopeY>-0.011 && slopeY<-0.006) return 1;
      break;
    case 16:
      if(slopeX>-0.006 && slopeX<-0.002 && slopeY>-0.022 && slopeY<-0.010) return 1;
      break;
    default:
      return 1;
    }
...
int PulseHeightsTrack (std::string const DataFileName, std::string const GainCalFileName, std::string const AlignmentFileName)
{
  ...
  PLTPlane::FiducialRegion FidRegionHits    = PLTPlane::kFiducialRegion_m2_m2;//PLTPlane::kFiducialRegion_Diamond;
  PLTPlane::FiducialRegion FidRegionCluster = PLTPlane::kFiducialRegion_m2_m2;//PLTPlane::kFiducialRegion_Diamond;
  ...
  for ( ; Event.GetNextEvent() >= 0; ++ientry) {
    ...
    for (size_t iTelescope = 0; iTelescope != Event.NTelescopes(); ++iTelescope) {
      ...
      if (Telescope->Plane(0)->NClusters() != 1 || Telescope->Plane(2)->NClusters() != 1 || (SuperCleanTracks && Telescope->Plane(1)->NClusters()!=1) ) {
	continue;
      }

      //std::cout<<"!!! "<<Telescope->Plane(0)->Cluster(0)->NHits()<<' '<<Telescope->Plane(1)->Cluster(0)->NHits()<<' '<<Telescope->Plane(2)->Cluster(0)->NHits()<<'\n';
      if(SuperCleanTracks && (Telescope->Plane(0)->Cluster(0)->NHits()!=1 || Telescope->Plane(1)->Cluster(0)->NHits()!=1 || Telescope->Plane(2)->Cluster(0)->NHits()!=1)){
	continue;
      }
	...
	// Keep only tracks in the signal region
	if (!SlopeSelection(Channel, Track)) continue;
        ...
        for (size_t icluster = 0; icluster < Track->NClusters(); ++icluster) {
          ...
          if (hOccupancyMap.count(id) == 0) {
            ...
            sprintf(BUFF, "Occupancy Ch%02i ROC%1i", Channel, ROC);
            ...
            // If we're making a new hist I'd say there's a 1 in 3 chance we'll need a canvas for it
            if (!cOccupancyMap.count(Channel)) {

              sprintf(BUFF, "Occupancy Ch%02i", Channel);
              cOccupancyMap[Channel] = new TCanvas(BUFF, BUFF, 1200, 1200);
              cOccupancyMap[Channel]->Divide(3,3);
