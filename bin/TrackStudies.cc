...
int TrackStudies(std::string const DataFileName, std::string const GainCalFileName, std::string const AlignmentFileName)
{
  ...
  std::map<int, int> hNTrackMaps;  
  std::map<int, int> hNClusterMaps;  
  std::map<int, int> hexNClusterMaps;  
  ...
  if(outFile.is_open()){
    ...
    for ( ; Event.GetNextEvent() >= 0; ++ientry) {
      ...
      for (size_t tel = 0; tel != Event.NTelescopes(); tel++) {
        ...
        if (!hNTrackMaps.count(chNo)) {
	  hNTrackMaps[chNo] = 0;
	  hNClusterMaps[chNo] = 0;
	  hexNClusterMaps[chNo] = 0;
	}
	...
	if (nhitplanes == 3) {
	  if(nclusters !=0) {
	    hNClusterMaps[chNo] += nclusters;
	  }

	  if(nclusters > 3) {
	    hexNClusterMaps[chNo] += nclusters;
	  }

	  if(ntracks !=0) {
	    hNTrackMaps[chNo] += ntracks;

	    //	    std::cout << "chNo: " << chNo << " " <<hNClusterMaps[chNo]<<" " << hNTrackMaps[chNo]<< " " << ntracks<< " " << hexNClusterMaps[chNo]<<std::endl;
	  }
	}
      }
	...
	//        for (std::map<int, TH1F*>::iterator it = hNClusterMap.begin(); it != hNClusterMap.end(); ++it) {
        ...
        for (std::map<int, int>::iterator it = hNClusterMaps.begin(); it != hNClusterMaps.end(); ++it) {

          int clCount = hNClusterMaps[it->first];
          int trCount = hNTrackMaps[it->first];
          int exClcount = hexNClusterMaps[it->first];

          double AvgNCluster = 1.0 * hNClusterMaps[it->first] / NEvents;
          double AvgNTrack = 1.0 * hNTrackMaps[it->first]/ NEvents;


          // reset counts           

	  hNTrackMaps[it->first] = 0;
	  hNClusterMaps[it->first] = 0;
	  hexNClusterMaps[it->first] = 0;
