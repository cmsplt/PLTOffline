////////////////////////////////////////////////////////////////////
//
//  TrackLumiZeroCounting -- a first attempt to derive a luminosity
//    using the rate of good tracks
//    Based on MeasureAccidentals (and uses a lot of common code
//    which I should refactor) to derive good tracks
		       ...
		       const std::string TrackDistributionFileName, const std::string TimestampFileName)
{
  ...
  // fill 5005
  const int nInjections = 11;
  int startBucket[nInjections] = {21, 2801, 3301, 4391, 12241, 13331, 17821, 21181, 22271, 30091, 31181};
  int numBunches[nInjections] = {1,  12,    72,   72,    72,   72,    1,     72,    72,    72,    72};

  // fill 5013
  //   const int nInjections = 29;
  //   int startBucket[nInjections] = { 621, 1111, 2201, 3291, 4381, 5471, 6561, 7861,
  // 				   8951, 10041, 11131, 12221, 13311, 14401, 15491, 16801, 
  // 				   17891, 18981, 20071, 21171, 22251, 23341, 24431, 25731,
  // 				   26821, 27911, 29001, 30091, 31181 };
  //   int numBunches[nInjections] = { 12, 72, 72, 72, 72, 72, 72, 72, 72, 72,
  // 				  72, 72, 72, 72, 72, 72, 72, 72, 72, 72,
  // 				  72, 72, 72, 72, 72, 72, 72, 72, 72 };

  int filledBunches[3564] = {0};
  // note: bunches internally are in 0-counting convention, so just (bucket-1)/10
  for (int i=0; i<nInjections; ++i) {
    int startBX = (startBucket[i]-1)/10;
    for (int j=startBX; j<startBX+numBunches[i]; ++j) {
      filledBunches[j] = 1;
    }
  }
  int nFilledBunches = 0;
  for (int i=0; i<3564; ++i) {
    if (filledBunches[i]) nFilledBunches++;
  }
...
//    return(0);
  ...
  std::vector<int> nEmptyEventsFilledBX(nSteps);
  std::vector<int> nNonEmptyFilledBX(nSteps);
  int nEventsByBX[3564] = {0};
  int nEmptyEventsByBX[3564] = {0};
  ...
  for (int ientry = 0; Event.GetNextEvent() >= 0; ++ientry) {
    ...
    nEvents[stepNumber]++;
    int nGoodTracksInEvent = 0; // actually wrongly named. but we only care about 0 or 1
	...
	if (foundOneGoodTrack) { 
	  ...
	  // ignore ch7 for fill 5005, since it dropped out midscan
	  // if (Telescope->Channel() != 7) nGoodTracksInEvent++;
	}
      }

    } // telescope loop

    nEventsByBX[Event.BX()]++;
    if (nGoodTracksInEvent == 0)
      nEmptyEventsByBX[Event.BX()]++;

    if (filledBunches[Event.BX()]) {
      nEventsFilledBX[stepNumber]++;
      if (nGoodTracksInEvent > 0)
	nNonEmptyFilledBX[stepNumber]++;
      else
	nEmptyEventsFilledBX[stepNumber]++;
    }
  ...
  for (int i=0; i<3564; ++i) {
    if (filledBunches[i] == 1) {
      std::cout << i+1 << " " << nEventsByBX[i] << " " << nEmptyEventsByBX[i] << std::endl;
    }
  }
