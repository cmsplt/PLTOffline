...
// CODE BELOW
 ...
 {  
   ...
   fit.resize(y_max);
   for (int m=0; m<y_max; m++){fit[m].resize(x_max);}
...
//Grab the plt event reader
  ...
  int NGraphPoints = 0; 
  int ientry = 0;
  int limit = 1000000000;//96000000;//000 /*139000000*/;

  std::string fname = "hist_"; 
  TH1F *hist = new TH1F((fname+Fill).c_str(), (fname+Fill).c_str(), NBins, XMin, XMax);
  TH1F *hist_all_data = new TH1F("hist_all_data", "hist_all_data", NBins, XMin, XMax);
  std::vector<std::vector<int>> pixfit0 = goodfitmap_vector(GainCalFileName, 22, 0); 
  std::vector<std::vector<int>> pixfit1 = goodfitmap_vector(GainCalFileName, 22, 1); 
  std::vector<std::vector<int>> pixfit2 = goodfitmap_vector(GainCalFileName, 22, 2); 
          ...
          if (NHits == 1) {
           ...
           if (id==220){
             if (col<52 && row<80){ //Sanity check 
                ...
                if(pixfit0[row][col]==1){
                    roc0=ThisClusterCharge; 
                    hist_all_data->Fill(ThisClusterCharge);
   ...
   // Calculate the avg charge that each pixel has recorded 
                ...
                if (pixfit0[row][col]==0) {    
                 ...
                 ++N22D[id][col][row];}
                  ...
                  //if (col<52 && row<80) if(pixfit1[row][col]==1) //This condition is too tight, if used for a triple concidence for ROC0
                  roc1 = ThisClusterCharge;
               }else if (id==222){
                  //if (col<52 && row<80) if(pixfit2[row][col]==1) //This condition is too tight, if used for a triple concidence for ROC0
 ...
 std::cout << "Events read: " << ientry+1 << std::endl;
  ...
  TFile *histogram_file = new TFile("ph_ch22_roc0.root", "UPDATE"); //change the name back to PH.root if you want to produce the peaks over lumi plot
  //hist->Scale(1/hist->GetEntries()); //Normalize Histogram
  //hist_all_data->Scale(1/hist_all_data->GetEntries());
 ...
 // f3D->Write();
  ...
  for (std::map<int, TCanvas*>::iterator it = cMap.begin(); it != cMap.end(); ++it) {
    it->second->SaveAs( TString("plots/") + it->second->GetName()+TString(".png") );
    delete it->second;
  }
