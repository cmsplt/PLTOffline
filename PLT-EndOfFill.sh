...
export PATH=$HOME/.local/bin:/afs/cern.ch/cms/lumi/brilconda-1.1.7/bin:$PATH # https://cms-service-lumi.web.cern.ch/cms-service-lumi/#prerequisite
...
cd ${localPLTOnline%/*}
svn co https://adelanno@svn.cern.ch/reps/cmsplt/trunk cmsplt
...
SLINKtimestamps(){
# Update /localdata/PLT-timestamps/PLT-timestamps.txt which lists fill|fillDeclared|fillEnd|slinkTS|workloopTS
printf '\n%s\n' "UPDATING SLINK TIMESTAMPS..."
...
brilcalc lumi -f ${fill} --byls --type PLTZERO --tssec               -o ${brilcalcLumiFile} # https://cms-service-lumi.web.cern.ch/cms-service-lumi/brilwsdoc.html#brilcalclumi
# Run BRILCALC BEAM to get number of colliding bunches
brilcalcBeamFile=/localdata/PLT-EndOfFill/brilcalc/${fill}.beam
brilcalc beam -f ${fill}                                             -o ${brilcalcBeamFile} # https://cms-service-lumi.web.cern.ch/cms-service-lumi/brilwsdoc.html#brilcalcbeam
...
for (( i=1; i<"${#LStstamp[@]}"; i++)); do
  printf "%s\n" "${i},1,${LStstamp[i]},${avgPU[i]}" >> ${SBILfile}
done
...
processGainCal(){
  ...
  ./GainCalFastFits                            ${gaincalDir}/gaincalfast_${mostRecentGaincalTS}.avg.txt
  \mkdir -p                                    ${gaincalDir}/${mostRecentGaincalTS}/
  \mv    -f ${localPLTOffline}/GainCalFits_*   ${gaincalDir}/${mostRecentGaincalTS}/
  \mv    -f ${localPLTOffline}/plots/GainCal/* ${gaincalDir}/${mostRecentGaincalTS}/
}
