...
namespace PLTU
{
  ...
  float peak_fromHisto(TH1* histo)
  {
    float peak_histo=0;
    int bin;
    for (int n=1; n<histo->GetNbinsX()+1; n++){
       if (histo->GetXaxis()->GetBinCenter(n)>8000){
         float temp = histo->GetBinContent(n); 
         if (temp>peak_histo){
            peak_histo=temp;
            bin = n; 
          }
     ...
     peak_histo = histo->GetXaxis()->GetBinCenter(bin);   
     return peak_histo;

  }

  float mean_fromHisto(std::vector<float>& V)
  { 
   float mean =0;
   TH1F *h = new TH1F("h", "h", 60, -1000, 50000);
   size_t i=0;
  ...
  float peak_finder (std::vector<float>& V)
  { 
    int maxV = 0;
    int minV = 0;
    ...
    int sqrt_size_V = ceil(sqrt(V.size()));  
    int bin_num = ceil((maxV-minV)/(sqrt_size_V));
    TH1F *h = new TH1F("h", "h", 60, -1000, 50000);
    ...
    for (int n=1; n<h->GetNbinsX()+1; n++){
       if (h->GetXaxis()->GetBinCenter(n)>=4525){   
         float temp = h->GetBinContent(n);                                                                                      
         if (temp>peak_histo){
            peak_histo=temp;
            bin = n;                                                                                                              }                                                                                                                     }                                                                                                                    }                                                                                                                       peak_histo = h->GetXaxis()->GetBinCenter(bin);
     return peak_histo;


 }  

 float fwhm (std::vector<float>& V)

 {
   TH1F *h = new TH1F("h", "h", 60, -1000, 50000);
    ...
    double fwhm_left = h->GetBinCenter(bin1);
    double fwhm_right = h->GetBinCenter(bin2);
    double fwhm = fwhm_right - fwhm_left;

    return fwhm;

 }
 float Fwhm (TH1* histo)
  {
  ...
  double peak_histo=0;
  double fwhm_left1=0;
  double fwhm_right1=0;                                                                                                                                              double fwhm1=0;                                                                                                                                                    std::vector<float> halfmax;                                                                                                                                        std::vector<int> position;                                                                                                                                         
  for (int n=0; n<histo->GetNbinsX()+1; n++){ 
   if (histo->GetXaxis()->GetBinCenter(n)>8000){ 
       float temp = histo->GetBinContent(n);                                                                                                                              if (temp>peak_histo){                                                                                                                                                 peak_histo=temp;
          bin = n;                                                                                                                                                         }
      }
    }
  for (int n=0; n<histo->GetNbinsX()+1; n++){
   if (histo->GetXaxis()->GetBinCenter(n)>8000){
     ...
     float hm = histo->GetBinContent(n);
     max2=histo->GetBinContent(bin)/2;                                                                                                                                  if (hm>=max2 ){                                                                                                                                                           halfmax.push_back(hm);
            position.push_back(n);
        }
     }
      }
    double value =halfmax.front();
    int bin1 = position.front();                                                                                                                                       int bin2 = histo->FindLastBinAbove(max2);
    fwhm_left1 = histo->GetBinLowEdge(bin1);

    fwhm_right1 = histo->GetBinLowEdge(bin2+1);
    fwhm1 = fwhm_right1 - fwhm_left1;
