...
//
// Modified on: Mar 2020 by Francesco Romeo (Vanderbilt University)
// Modifications:
// . 1D plots are now for ADC vs vCal (much easier to fit)
// . Added error on ADC
// . Fit function modified
// . Good and bad fit properly defined and saved
//
////////////////////////////////////////////////////////////////////
...
int GainCalFastFits (TString const InFileName)
{
  ...
  std::map<int, std::vector< std::pair<float, float> > > MapErr;
  ...
  for (std::string line; std::getline(f, line); ) {
    ...
    // Limit yourself on a given pixel
    //if (!(channel==2 && roc==0 && col==0 && row==0)) continue;
    //if (!(channel==8 && roc==0 && col==3)) continue;
    //if (!(channel==8 && roc==0)) continue;
    //if (!(channel==8 && roc==0 && col==2 && row==30)) continue;
    //if (!(channel==8 && roc==0 && col<=100)) continue;
    //if (!(channel==8 && roc==0 && col==7 && row==1)) continue;
    //if (!(channel==8)) continue;

    int Id = Pack(channel, roc, col, row);
    Map[Id].push_back( std::make_pair(adc, vcal) );
    MapErr[Id].push_back( std::make_pair(adcErr, 0) );
  ...
  gooddirname.Form("Fits_Good_Channel_ROC");
  baddirname.Form("Fits_Bad_Channel_ROC");
  TDirectory* dGoodFits = fOutRoot.mkdir(gooddirname);
  TDirectory* dBadFits = fOutRoot.mkdir(baddirname); 

  // Loop over all entries in the map (which is by definiteion organized by pixel already, how nice
  std::map<int, std::vector< std::pair<float, float> > >::iterator ItErr = MapErr.begin();
  for (std::map<int, std::vector< std::pair<float, float> > >::iterator It = Map.begin(); It != Map.end(); ++It) {
    ...
    */
    ...
    float XErr[It->second.size()];
    float YErr[It->second.size()];
    for (size_t i = 0; i != It->second.size(); ++i) {
      X[i] = It->second[i].first;
      Y[i] = It->second[i].second;
      XErr[i] = ItErr->second[i].first;
      YErr[i] = ItErr->second[i].second;
    }
    */
    // Define the function we will fit for each pixel (inside the for loop, or might have problems inheriting as initial parameters values those of previous fit, though it should not happen)
    TF1 FitFunc("FitFunc", "[0]/([1]-TMath::Exp(-[2]*(x-[3])))+[4]", 0, 1000); 
    float Y[It->second.size()];
    float X[It->second.size()];
    float YErr[It->second.size()];
    float XErr[It->second.size()];
    ...
    for (size_t i = 0; i != It->second.size(); ++i) {
      Y[i] = It->second[i].first;
      X[i] = It->second[i].second;
      YErr[i] = ItErr->second[i].first;
      XErr[i] = ItErr->second[i].second;
      //std::cout<<X[i]<<" "<<XErr[i]<<" "<<Y[i]<<" "<<YErr[i]<<std::endl;
    }

    // Actually make a TGraph
    TGraphErrors g(It->second.size(), X, Y, XErr, YErr);
    TString const Name = TString::Format("Fit_Channel%i_ROC%1i_Col%2i_Row%2i", channel, roc, col, row);
    g.SetName(Name);
    g.SetTitle(Name);
    g.GetYaxis()->SetTitle("ADC");
    g.GetXaxis()->SetTitle("VCal");

    // Get the min and max point from the graph
    g.GetPoint(0, vcalMin, adcMin);
    g.GetPoint(g.GetN()-1, vcalMax, adcMax);

    // Set the range of the fit
    FitFunc.SetRange(vcalMin,vcalMax);
    /* //Better if it finds initial values alone
    FitFunc.SetParameter(0, (adcMax-adcMin));
    FitFunc.SetParameter(1, 1);
    FitFunc.SetParameter(2, 0.05);
    FitFunc.SetParameter(3, vcalMin);
    FitFunc.SetParameter(4, adcMax);
    */

    // Do the fit
    //See "access to the fit status" in https://root.cern.ch/doc/master/classTGraph.html#a61269bcd47a57296f0f1d57ceff8feeb
    int FitResult = g.Fit("FitFunc", "Q","",vcalMin,vcalMax);
    ...
    //Re-iterate the fit
    int maxNumiteration = 20;
    int currIteration = 0;
    bool nextIteration = true;
    float threshold = 0.01;
    while(currIteration<maxNumiteration && nextIteration){
     //std::cout<<col<<" "<<row<<" "<<currIteration<<" "<<Param[0]<<" "<<Param[1]<<" "<<Param[2]<<" "<<Param[3]<<" "<<Param[4]<<std::endl;
     FitFunc.SetParameter(0, Param[0]);
     FitFunc.SetParameter(1, Param[1]);
     FitFunc.SetParameter(2, Param[2]);
     FitFunc.SetParameter(3, Param[3]);
     FitFunc.SetParameter(4, Param[4]);
     FitResult = g.Fit("FitFunc", "Q","",vcalMin,vcalMax);
     if(fabs(FitFunc.GetParameter(0)-Param[0])/Param[0]<threshold && fabs(FitFunc.GetParameter(1)-Param[1])/Param[1]<threshold && fabs(FitFunc.GetParameter(2)-Param[2])/Param[2]<threshold && fabs(FitFunc.GetParameter(3)-Param[3])/Param[3]<threshold && fabs(FitFunc.GetParameter(4)-Param[4])/Param[4]<threshold){
      nextIteration = false;
     }else{
      Param[0] = FitFunc.GetParameter(0);
      Param[1] = FitFunc.GetParameter(1);
    ...
    if(FitResult == 0 && (adcMax-adcMin)>5 && It->second.size()>5 && vcalMin<=85 && vcalMax>=795){
      ...
      //std::cout<<"Good "<<row<<" "<<FitResult<<" "<<FitFunc.GetChisquare()<<" "<<FitFunc.GetNDF()<<" "<<FitFunc.GetChisquare()/FitFunc.GetNDF()<<" "<<(adcMax-adcMin)<<" "<<Param[0]<<" "<<Param[1]<<" "<<Param[2]<<" "<<Param[3]<<" "<<Param[4]<<std::endl;
    } //end FitResult == 0

    else {
      //std::cout<<"Bad "<<col<<" "<<row<<" "<<FitResult<<" "<<FitFunc.GetChisquare()<<" "<<FitFunc.GetNDF()<<" "<<FitFunc.GetChisquare()/FitFunc.GetNDF()<<" "<<(adcMax-adcMin)<<" "<<Param[0]<<" "<<Param[1]<<" "<<Param[2]<<" "<<Param[3]<<" "<<Param[4]<<std::endl;
