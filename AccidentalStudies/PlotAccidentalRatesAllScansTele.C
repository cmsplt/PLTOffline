...
"CombinedRatesTele4444_Align4444.txt",
"CombinedRatesTele4555_Align4444.txt"
};
...
//  int nscopes = 14;
  ...
  for (int i=0; i<nsteps; ++i) {
        fscanf(rfile, "%d %d %d %d %d %d %d %d %d %d", &tBegin, &tEnd, &nTrig, &tracksAll[0], &tracksGood[0], &tracksAll[1], &tracksGood[1], &tracksAll[2], &tracksGood[2], &tracksAll[3]);

        fscanf(rfile, "%d %d %d %d %d %d %d %d %d %d", &tracksGood[3], &tracksAll[4], &tracksGood[4], &tracksAll[5], &tracksGood[5], &tracksAll[6], &tracksGood[6], &tracksAll[7], &tracksGood[7], &tracksAll[8]); 
  ...
  // Process the data. Change index in tracks vectors to choose proper scope.
    ...
    accidentalRateErr.push_back(100.0*sqrt(accrate*(1-accrate)/tracksAll[0]));
...
void PlotAccidentalRatesAllScansTele(void) {
  // style from PLTU
