#Project6B/bis: The goal is to get an histogram of the normalized rates for all the channels across just one chosen fill to start understanding how each channel behaves.
#Beg date 22/09/2017
    ...
    for row in agg_zero.iterrows():
        ...
        if channel == 15:
            ...
            if N==0:          #The nibbles where <N>=0 cannot be properly normalized
                ...
                bx_norm = [(x-N)/(x**.5) for x in total_bx if x>0]
                if len(bx_norm)==14:
...
for channel in range(14):
    ...
    #histogram of the scores of the channel
    plt.hist(ch_sample, "auto")
