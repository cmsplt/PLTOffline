...
@timerDecorator
def findHistogramFile( scanStart:pandas.Timestamp, scanEnd:pandas.Timestamp ) -> t.List[str]:
...
def calculateFileSeek( histFilename:str, scanStart:pandas.Timestamp, scanEnd:pandas.Timestamp ) -> t.Tuple[float,float]:
...
def scanStartDate( scanHist:pandas.DataFrame, histFilename:str ): # -> datetime.date:
    ...
    currentDate = histFileDate( histFilename ) # date from filename
    ...
    if scanStartTime < histFileStartTime: currentDate += pandas.Timedelta(days=1)
...
def processScanHist( histFilename:str, histSeek:t.Tuple[float,float], scanStart:pandas.Timestamp ) -> pandas.DataFrame:
...
def mergeDF( scanVmon:pandas.DataFrame, scanHist:pandas.DataFrame, ch:int ) -> pandas.DataFrame:
    ...
    chVmon:t.Dict[int,str] = { **chHV('mN',0), **chHV('mF',4), **chHV('pN',8), **chHV('pF',12) } # [https://twiki.cern.ch/twiki/bin/viewauth/CMS/PLT#PLT_Channel_Map]
...
def processScan( scan:str, scanBoundaries:t.List ) -> pandas.DataFrame:
