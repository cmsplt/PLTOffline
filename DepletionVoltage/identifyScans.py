...
def MapLogFile( logfile:str ) -> pandas.DataFrame:
    ...
    with open( logfile, 'r', encoding='utf-8-sig' ) as logFile:
        ...
        lines:List[str] = list( map( PreprocessLine, logFile.readlines() ) )
    logData:List[List[str]] = [ list(channelData) for delim,channelData in itertools.groupby( lines, lambda z: z=='' ) if not delim ]
        # group logfile data into a sublist for each channel ( len(logData)==16 ) using an empty string ('') as delimiter
            # empty strings correspond to newlines in the logfile but are stripped to '' using line.rstrip( '\n' ) in PreprocessLine( line )
    ...
    channels:List[str] = [ dataCh[0].split('/')[-1] for dataCh in logData ]
        # list channels headers (sub-string after last '/' for the first entry in each sub-list)
    logDict:Dict[str,Dict[str,float]] = { channels[ch] : { entry.split(',')[0] : float( entry.split(',')[1] ) for entry in dataCh[2:] } for ch,dataCh in enumerate(logData) }
...
def FilterLogFile( logfile:str ) -> pandas.DataFrame:
    ...
    logData:Dict[str,Dict[str,float]] = {}
    for line in lines:
        if 'CMS_PLT' in line:
            ch:str = line.split('/')[-1]
            logData[ch]:Dict[str,float] = {}
...
def BrilcalcTimestamps( beginTS:str, endTS:str ) -> pandas.DataFrame:
    ...
    try: output:List[str] = subprocess.check_output( shlex.split(command), encoding='utf-8' ).splitlines()
    ...
    except subprocess.CalledProcessError: output:List[str] = ['','#run:fill,ls,time,beamstatus,E(GeV),delivered(/ub),recorded(/ub),avgpu,source']
    header:List[str] = output[1].split(',')
    lsData:List[str] = [ line.split(',') for line in output[2:-4] ]
    ...
    df[flCol] = df[flCol].apply( lambda col: col.astype(float) )
...
def PlotScanCandidate( scanCandidate:pandas.DataFrame, year:int ):
    ...
    plot = scanCandidate.plot( legend=False )
...
@TimerDecorator
def filterScanCandidates( logDF:pandas.DataFrame, timestamps:pandas.Series ) -> List[pandas.DataFrame]:
