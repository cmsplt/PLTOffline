...
defineFunctions(){
    ...
    gitInstallCompile(){
        ...
        local root5="/cvmfs/cms.cern.ch/slc5_amd64_gcc481/cms/cmssw/CMSSW_7_1_1/external/slc5_amd64_gcc481/bin/thisroot.sh"
        [[ "${gitRepo}" = "PLTOffline" ]] && source "${root5}" && make # try to restore "$PATH" from "PATHbkp=$PATH"?
        cd "${wDir}"
    }
