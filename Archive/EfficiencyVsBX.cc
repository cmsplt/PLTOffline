...
void EfficiencyVsBX(int channel, int fit_dim_, int doBgdSubtraction=1) {
  fit_dim=fit_dim_;
  if(fit_dim != 1 && fit_dim != 2 && fit_dim != 3) return;
  TFile *f = new TFile("hit_data.root","OPEN");
  TTree *data = (TTree*)f->Get("data");
  int eventNo, eventTime, BXNo, roc1_trial, roc1_success;
  float slopeX, slopeY;
  switch(channel){
  case 13:
  ...
  TH1F *hTrial = new TH1F("hTrial","Trials",nbins,1,nbins);
  TH1F *hSuccess = new TH1F("hSuccess","Successes",nbins,1,nbins);
  ...
  for(int i=0; i<data->GetEntries(); ++i){
    ...
    if(slopeX > x_min && slopeX < x_max && slopeY > y_min && slopeY < y_max){
      hTrial->Fill(BXNo,roc1_trial);
      hSuccess->Fill(BXNo,roc1_success);
      NeventsInSignalRegion += roc1_trial;
    }
    NeventsTotal += roc1_trial;
    int binNo = hTrial->FindBin(BXNo)-1;
    if(binNo>=0 && binNo<nbins && roc1_trial > 0){
  ...
  TCanvas *c1 = new TCanvas("c1","Efficiencies vs. BX", 800,800);

  float max_eff = 0.0; float min_eff=1.0;
  int lastBX = 0;
  TGraphErrors *gEff = new TGraphErrors(nbins);
  for(int i=0; i<nbins; ++i){
    float bin_center = hTrial->GetBinLowEdge(i+1);
    float bin_error = 0;//hTrial->GetBinWidth(i+1)/2.0;
    if(Ntrial[i]>0) {
      if( (channel==13 &&(Nsuccess[i]<200 || Ntrial[i] <300)) || (channel==24 &&(Nsuccess[i]<100 || Ntrial[i] <200))){
	//if(Nsuccess[i]<=10 || Ntrial[i] <10){
	gEff->SetPoint(i,bin_center,0);
	gEff->SetPointError(i,bin_error,0.25);
      }
      else{
	gEff->SetPoint(i,bin_center,eff_sig[i]);
	gEff->SetPointError(i,bin_error,delta_eff_sig[i]);
	cout<<"Active BX: "<<i<<", space = "<<i-lastBX<<endl;
	lastBX = i;
      }
  ...
  gEff->GetXaxis()->SetTitle("BX Index");
  ...
  if(fit_dim==1) sprintf(title,"plots/EfficiencyVsBX_SlopeX_Ch%i.png",channel);
  else if(fit_dim==2) sprintf(title,"plots/EfficiencyVsBX_SlopeY_Ch%i.png",channel);
  else if(fit_dim==3) sprintf(title,"plots/EfficiencyVsBX_Slope2D_Ch%i.png",channel);
  c1->SaveAs(title);

  TCanvas *c1_zoom = new TCanvas("c1_zoom","Efficiencies vs. BX zoomed", 800,800);
  TGraphErrors *gEffActiveBX = new TGraphErrors();
  TGraphErrors *gEffInactiveBX = new TGraphErrors();
  gEffActiveBX->SetMarkerColor(3);
  gEffInactiveBX->SetMarkerColor(2);
  int iPointActiveBX = 0, iPointInactiveBX = 0;
  for(int i = 130; i<=230; ++i){
    if(i%2==0 && i >= 146 && i<=216){
      if(Ntrial[i-1] > 0){
	gEffActiveBX->SetPoint(iPointActiveBX,i,eff_sig[i-1]);
	gEffActiveBX->SetPointError(iPointActiveBX,0,delta_eff_sig[i-1]);
	iPointActiveBX++;
      }
      else{
	gEffActiveBX->SetPoint(iPointActiveBX,i,0);
	gEffActiveBX->SetPointError(iPointActiveBX,0,0.25);
	iPointActiveBX++;
      }
    }
    else{
      if(Ntrial[i-1] > 0){
	gEffInactiveBX->SetPoint(iPointInactiveBX,i,eff_sig[i-1]);
	gEffInactiveBX->SetPointError(iPointInactiveBX,0,delta_eff_sig[i-1]);
	iPointInactiveBX++;
      }
      else{
	gEffInactiveBX->SetPoint(iPointInactiveBX,i,0);
	gEffInactiveBX->SetPointError(iPointInactiveBX,0,0.25);
	iPointInactiveBX++;
      }
    }
  }

  //gEffActiveBX->Draw("AP");
  //gEffInactiveBX->Draw("P");
  gEff->GetXaxis()->SetRangeUser(zoomLow,zoomHigh);
  gEff->Draw("AP");
  if(fit_dim==1) sprintf(title,"plots/EfficiencyVsBX_SlopeX_Ch%i_zoom.png",channel);
  else if(fit_dim==2) sprintf(title,"plots/EfficiencyVsBX_SlopeY_Ch%i_zoom.png",channel);
  else if(fit_dim==3) sprintf(title,"plots/EfficiencyVsBX_Slope2D_Ch%i_zoom.png",channel);
  c1_zoom->SaveAs(title);
  ...
  gNsuc->GetXaxis()->SetTitle("BX Index");
  gNsuc->Draw("AP");
  ...
  if(doBgdSubtraction){
    ...
    gNbgd->GetXaxis()->SetTitle("BX Index");
    gNbgd->Draw("AP");
    c2->cd(3);
  }
  ...
  gNtri->GetXaxis()->SetTitle("BX Index");
  gNtri->Draw("AP");
  ...
  if(doBgdSubtraction){
    ...
    gBgdE->GetXaxis()->SetTitle("BX Index");
    gBgdE->Draw("AP");
  ...
  TCanvas *c2_zoom = new TCanvas("c2_zoom","Cross Checks Zoomed",800,800); c2_zoom->Divide(2,2);
  c2_zoom->cd(1);
  gNsuc->GetXaxis()->SetRangeUser(zoomLow,zoomHigh);
  gNsuc->Draw("AP");
  c2_zoom->cd(2);
  gNbgd->GetXaxis()->SetRangeUser(zoomLow,zoomHigh);
  gNbgd->Draw("AP");
  c2_zoom->cd(3);
  gNtri->GetXaxis()->SetRangeUser(zoomLow,zoomHigh);
  gNtri->Draw("AP");
  c2_zoom->cd(4);
  gBgdE->GetXaxis()->SetRangeUser(zoomLow,zoomHigh);
  gBgdE->Draw("AP");
  if(fit_dim==1) sprintf(title,"plots/EfficiencyCrossChecks_SlopeX_Ch%i_zoom.png",channel);
  else if(fit_dim==2) sprintf(title,"plots/EfficiencyCrossChecks_SlopeY_Ch%i_zoom.png",channel);
  else if(fit_dim==3) sprintf(title,"plots/EfficiencyCrossChecks_Slope2D_Ch%i_zoom.png",channel);
  c2_zoom->SaveAs(title);
...
int main (int argc, char* argv[])
{
  ...
  EfficiencyVsBX(ChNo, FitDim, BgdSub);
  return 0;
}
